...
const FLOOR_CALENDAR_NAME = "Lighthill Floor";
...
async function ensureFloorCalendar(accessToken: string): Promise<string> {
  const list = await googleJson<{ items?: Array<{ id: string; summary: string }> }>(
    "https://www.googleapis.com/calendar/v3/users/me/calendarList?minAccessRole=owner",
    { headers: { Authorization: `Bearer ${accessToken}` } },
  );
  const existing = list.items?.find((item) => item.summary === FLOOR_CALENDAR_NAME);
  ...
  const created = await googleJson<{ id: string }>("https://www.googleapis.com/calendar/v3/calendars", {
    ...
    body: JSON.stringify({ summary: FLOOR_CALENDAR_NAME, timeZone: STUDIO_TZ }),
  });
  return created.id;
}
...
export async function completeGoogleConnect(
  ...
  const calendarId = await ensureFloorCalendar(tokens.access_token);
  const sql = await getSql();
  await sql`
