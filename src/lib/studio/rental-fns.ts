...
export const listRentalAvailability = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }) => {
    ...
    for (let i = 0; i < DAYS_AHEAD; i += 1) {
      ...
      const slots = slotsForDay(date, data.durationMinutes, rules.buffer, busy, now);
...
export const startRentalCheckout = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const noteParts = [
      "Public rental. 50% deposit via Stripe.",
      data.notes?.trim() || "",
    ].filter(Boolean);
...
export async function fulfillStripeSession(sessionId: string): Promise<{ ok: boolean; bookingId?: string }> {
  ...
  if (!existingInvoice[0]) {
    const addons = Array.isArray(booking.addons)
      ? (booking.addons as AddonSelection[])
      : [];
    const quote = quoteBooking({
      kind: "rental",
...
export const confirmRentalCheckout = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }) => {
    const result = await fulfillStripeSession(data.sessionId);
